		...
		*/
		async function onAnyEvent() {
			...
			for (const key of allOnAnyEvent) {
				...
				const getText2 = createGetText2(langCode, `${process.cwd()}/NEROBOT/longbot/events/${langCode}.js`, prefix, command);
		...
		*/
		async function handlerEvent() {
			...
			for (const [key] of allEventCommand) {
				...
				const getText2 = createGetText2(langCode, `${process.cwd()}/NEROBOT/longbot/events/${langCode}.js`, prefix, getEvent);
				const time = getTime("DD/MM/YYYY HH:mm:ss");
				try {
