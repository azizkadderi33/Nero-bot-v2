...
module.exports = async function (api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, createLine) {
	...
	for (const folderModules of folders) {
		const header = folderModules === "cmds" ? createLine("LOAD COMMANDS") : createLine("LOAD EVENTS");
		console.log(chalk.hex("#00ff00").bold(header)); // green header
		...
		const fullPathModules = path.normalize(process.cwd() + `/NEROBOT/SystemNero/${folderModules}`);
		...
		// ----- SHOW SUMMARY -----
		if (commandLoadSuccess > 0) console.log(chalk.green.bold(`\nSuccessfully loaded ${commandLoadSuccess} ${text}(s)`));
		if (commandError.length > 0) {
			console.log(chalk.red.bold(`Failed to load ${commandError.length} ${text}(s):`));
			for (const item of commandError) console.log(chalk.red(`✖ ${item.name}: ${item.error.message}`));
		}
		console.log("\n");
	}
};
